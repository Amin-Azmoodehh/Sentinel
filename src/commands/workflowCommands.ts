  ...
  workflowCommand
    ...
    .action(async () => {
      ...
      context.patterns.forEach(pattern => {
        log.info(`  - ${pattern.type}: ${pattern.imports.length} imports, ${pattern.exports.length} exports`);
      });
    });
  ...
  workflowCommand
    ...
    .action(async (options: { config: string }) => {
      ...
      try {
        ...
        log.error(`❌ Batch execution failed: ${error instanceof Error ? error.message : String(error)}`);
        process.exit(1);
      }
    });
};
